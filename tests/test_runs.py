...
def test_get_all_users_runs():
    run_1_uid = 3
    pl_1 = {"user_id": 3, "date": "2026-04-23", "distance": 1.5}
    pl_2 = {"user_id": 3, "date": "2026-05-14", "distance": 8.5}
    pl_3 = {"user_id": 4, "date": "2026-05-14", "distance": 21}

    def create_runs(payload):
        resp_1 = client.post("/runs", json=payload)
        assert resp_1.status_code == 200
        return resp_1.json()
    
    p1_data = create_runs(pl_1)
    p2_data = create_runs(pl_2)
    p3_data = create_runs(pl_3)


    # resp_1 = client.post("/runs", json=pl_1)
    # assert resp_1.status_code == 200
    # p1_data = resp_1.json()

    # resp_2 = client.post("/runs", json=pl_2)
    # assert resp_2.status_code == 200
    # p2_data = resp_2.json()

    # resp_3 = client.post("/runs", json=pl_3)
    # assert resp_3.status_code == 200
    # p3_data = resp_3.json()

    resp_4 = client.get(f"/runs/user/{run_1_uid}")
    assert resp_4.status_code == 200
    print("resp_4", resp_4.json())
    runs = resp_4.json()

    for run in runs:
        assert run["user_id"] == 3

    client.delete(f"/runs/{p1_data['id']}")
    client.delete(f"/runs/{p2_data['id']}")
    client.delete(f"/runs/{p3_data['id']}")
